...
def research_and_scrape(query, criteria_string, end_page = 2):
    ...
    for page in range(2, end_page + 1):
        print('We start to scrape page n° {}'.format(page))
        search_url = 'https://www.vinted.fr/vetements?search_text=' + query.replace(' ', '%20') + criteria_string + '&page={}'.format(page) # 
        DATA_ALL = DATA_ALL.append(scrape_research_page(search_url))
        print('We finished to scrape page n° {}'.format(page))
...
def collect_info(page_url):
    '''Date de l’annonce, Prix, Marque, Taille, ÉTAT, Couleur, Emplacement, Nombre de Vues, 
    Nombre d’intéressés, catégorie'''
    driver.get(page_url)
    #time.sleep(4)
    ...
    try:
        brand = clean_data[np.where([(y[:6]=='MARQUE') for y in clean_data])[0][0]][6:].strip() # obtain brand without any spaces
    except:
        brand = 'Na'

    try:
        colors = clean_data[np.where([(y[:7]=='COULEUR') for y in clean_data])[0][0]][7:].strip() 
    except:
        colors = 'Na'

    try:
        size = clean_data[np.where([(y[:6]=='TAILLE') for y in clean_data])[0][0]][6:].strip() 
    except:
        size = 'Na'

    try:
        condition = clean_data[np.where([(y[:4]=='ÉTAT') for y in clean_data])[0][0]][4:].strip() 
    except:
        condition = 'Na'

    try:
        views = clean_data[np.where([(y[:14]=='NOMBRE DE VUES') for y in clean_data])[0][0]][14:].strip() 
    except:
        views = '0'

    try:
        interested = re.split(' ',clean_data[np.where([(y[:14]=='INTÉRESSÉS·ÉES') for y in clean_data])[0][0]][14:].strip())[0]
    except:
        interested = '0'

    try:
        location = clean_data[np.where([(y[:11]=='EMPLACEMENT') for y in clean_data])[0][0]][11:].strip() 
    except:
        location = 'Na'
    ...
    try:
        description = clean_data[np.where([(y[:13]=='AJOUTÉ IL Y A') for y in clean_data])[0][0] + 1].strip() 
    except:
        description = 'Na'
