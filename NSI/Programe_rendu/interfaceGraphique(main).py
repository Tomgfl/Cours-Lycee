...
def print_item(article,ID):
    ...
    #placement (max 13 articles par ligne)
    ID_x = ID%13
    ID_y = ID//13
...
def page_suivante(ID,allArticles):
    if ID+39 <= len(allArticles):
        print_page(ID+39,allArticles)
    
def page_precedente(ID,allArticles):
    if ID-39 >= 0:
        print_page(ID-39,allArticles)
...
def recupere():
    ...
    print_page(0,allArticles)
    
    
    
            
def print_page(ID,allArticles):
    global Frame_filtres
    if len(allArticles) > 39:
        for i in range(39):
            print_item(allArticles[i+ID],i)
    ...
    B = Button(Frame_filtres, text = "Suivant", command=partial(page_suivante,ID,allArticles))
    B.grid(row=0,column=6)
    
    B2 = Button(Frame_filtres, text = "Precedent", command=partial(page_precedente,ID,allArticles))
